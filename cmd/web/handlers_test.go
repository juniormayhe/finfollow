...
func TestPingUnitTest(t *testing.T) {
...
func TestPingIntegration(t *testing.T) {
	...
	// them).
	app := &application{
		errorLog: log.New(ioutil.Discard, "", 0), // loggers are needed by the logRequest and recoverPanic middlewares
	...
	// the test finishes.
	ts := httptest.NewTLSServer(app.routes()) //app.routes is our http.Handler, and this handler gets called each time the test server receives a HTTPS request
	//we can use instead httptest.NewServer() when testing HTTP server
	defer ts.Close()
